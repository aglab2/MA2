...
s32 perform_ground_step(struct MarioState *m) {
    ...
    Vec3f intendedPos;
    int numSteps = 16;
    const f32 minVel = 0.000099658966f; // 38d10000
    const struct Surface* cachedFloor = NULL;
    f32 cachedNormalXdY, cachedNormalZdY;
    ...
    for (i = 0; i < numSteps; i++) {
        ...
        f32 velMag2 = sqr(velX) + sqr(velZ);
        if (velMag2 > minVel)
        {
            if (cachedFloor != floor)
            {
                cachedFloor = floor;
                cachedNormalXdY = floor->normal.x / floor->normal.y;
                cachedNormalZdY = floor->normal.z / floor->normal.y;
            }

            f32 divi = sqr(velX * cachedNormalXdY + velZ * cachedNormalZdY) / velMag2;
            mult = 1.f / sqrtf(1.f + divi);
        }

        intendedPos[0] = m->pos[0] + velX * mult;
        intendedPos[2] = m->pos[2] + velZ * mult;
...
static struct Surface *check_ledge_grab(struct MarioState *m, struct Surface *prevWall, struct Surface *wall, Vec3f intendedPos, Vec3f nextPos, Vec3f ledgePos, struct Surface **ledgeFloor) {
    ...
    if (ledgeFloor == NULL
        ...
        || absf((*ledgeFloor)->normal.y) < 0.15f // H64 TODO: check if floor is actually slippery
#endif
    ) {
        return NULL;
    }
...
static s32 perform_air_quarter_step(struct MarioState *m, Vec3f intendedPos, u32 stepArg) {
    ...
    resolve_and_return_wall_collisions(nextPos, 150.0f, 52.0f, &upperWall);
