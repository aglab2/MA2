...
extern void print_text_fmt_int(s32 x, s32 y, const char *str, int d);
...
Gfx *geo_update_transparency(s32 callContext, struct GraphNode *node, UNUSED void *context) {
    Gfx *dlStart = NULL;
    struct GraphNodeGenerated *currentGraphNode = (struct GraphNodeGenerated *) node;
...
Gfx *geo_switch_ccs(s32 callContext, struct GraphNode *node, UNUSED void *context) {
    if (callContext == GEO_CONTEXT_RENDER) {
        struct GraphNodeSwitchCase *switchCase = (struct GraphNodeSwitchCase *) node;
        if (switchCase->numCases)
        {
            ...
            switch (time)
            {
                ...
                case 15:
                case 16:
                    switchCase->selectedCase = time - 9;
                    break;
            }
        }
        else
        {
            int time = gFlipbookTimer % 16;
            switchCase->selectedCase = CLAMP(time, 0, 5);
        }
    }
...
struct Object *cur_obj_find_object_with_behavior_and_bparam3(const BehaviorScript *behavior, int val) {
    uintptr_t *behaviorAddr = segmented_to_virtual(behavior);
    struct ObjectNode *listHead = &gObjectLists[get_object_list_from_behavior(behaviorAddr)];
    struct Object *obj = (struct Object *) listHead->next;
    while (obj != (struct Object *) listHead) {
        if (obj->behavior == behaviorAddr
            && obj->activeFlags != ACTIVE_FLAG_DEACTIVATED
            && obj != o
            && GET_BPARAM3(obj->oBehParams) == val
        ) {
            return obj;
        }
