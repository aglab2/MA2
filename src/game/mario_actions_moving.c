...
void update_sliding_angle(struct MarioState *m, f32 accel, f32 lossFactor) {
    ...
    if (m->forwardVel > 160.0f) {
        m->slideVelX = m->slideVelX * 160.0f / m->forwardVel;
        m->slideVelZ = m->slideVelZ * 160.0f / m->forwardVel;
    }
...
s32 begin_braking_action(struct MarioState *m) {
    ...
    if (m->forwardVel >= 16.0f && absf(m->floor->normal.y) >= COS80) {
...
s32 act_hold_walking(struct MarioState *m) {
    ...
    if (m->intendedMag - m->forwardVel > 10.0f) {
        m->particleFlags |= PARTICLE_DUST;
    }
...
void common_slide_action(struct MarioState *m, u32 endAction, u32 airAction, s32 animation) {
    ...
    if (must_cancel_landing(m)) {
        return (void) mario_push_off_steep_floor(m, ACT_FREEFALL, 0);
    }
...
int must_cancel_landing(struct MarioState* m)
{
    static u64 Flags = (1ULL << (LEVEL_CE  - LEVEL_CE))
                     ...
                     | (1ULL << (LEVEL_MHE - LEVEL_CE))
                     | (1ULL << (LEVEL_DL  - LEVEL_CE))
                     | (1ULL << (LEVEL_SO  - LEVEL_CE))
                     | (1ULL << (LEVEL_EQ  - LEVEL_CE))
